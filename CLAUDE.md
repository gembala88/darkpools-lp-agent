...
| `LPAGENT_API_KEY` | No | LPAgent API access |
| `BIRDEYE_API_KEY` | No | Birdeye API for token data |
| `JUPITER_API_KEY` | No | Jupiter API for swaps and routing |
| `JUPITER_REFERRAL_ACCOUNT` | No | Jupiter referral account address |
| `JUPITER_REFERRAL_FEE_BPS` | No | Jupiter referral fee basis points |
| `GMGN_API_KEY` | No | GMGN API for token screening |
| `PUBLIC_API_KEY` | No | Public API access key |
| `AGENT_MERIDIAN_API_URL` | No | Agent Meridian API endpoint |
| `ENVRYPT_KEY` | No | Encryption key for env |
| `ENVCRYPT_KEY` | No | Alternate encryption key |
| `LOG_LEVEL` | No | Log level (debug/info/warn/error) |
| `TELEGRAM_ALLOWED_USER_IDS` | No | Comma-separated allowed Telegram user IDs |
| `ALLOW_SELF_UPDATE` | No | Allow self-update of agent code |
